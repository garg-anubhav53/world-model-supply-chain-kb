...
29. **NEW (Fire 7) — gate-check 3 fresh China motor/joint-module leads**: Moons' Electric (603728.SH), Fulin Precision (富临精工, confirm parent ticker), Bukom (步科股份) — all named Zhiyuan/AgiBot vendors surfaced incidentally by the Leili capture-rate check. HIGH PRIORITY, reopens a vector previously declared exhausted.
30. **NEW (Fire 7) — adjacent verticals (drone/defense/agri/medical/maritime/mining/logistics/space)**: BLIND and now idle 7 fires — forced-next per the coverage-ledger's own 3-fire-idle rule. Share the same forced sensor/actuator/compute inputs as R/AV; untested sourcing angle.
31. **NEW (Fire 7) — Hota Industrial reduction-gear corner**: now fully KILLED — if a genuinely NEW Taiwan/Japan reduction-gear name with an un-priced, un-confirmed humanoid link surfaces, it needs a fresh causal chain, not a re-run of Hota's.

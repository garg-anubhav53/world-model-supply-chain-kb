...
## Sourcing-method prior: buyer-disclosure beats capital-structure (added Fire 92, after a 4-for-4 negative wildcard test)
When evaluating a candidate NEW sourcing angle, weight it by document type first: does it disclose a **named buyer-seller transaction** (supply contract, IR customer-concentration filing, prospectus supplier appendix — this campaign's historically most productive document type) or merely a **named capital relationship** (a shareholder registry, a securitization/ABS counterparty, an IPO cornerstone-investor list)? Fire 92 tested 4 genuinely new angles in the latter category (China supply-chain ABS prospectuses, HKEX cornerstone-investor lists, Chinese state guidance-fund direct portfolios, sell-side brokerage reports) and all 4 came back structurally unproductive for the same underlying reason — capital relationships surface either still-private companies (no ticker) or already-large incumbents, rarely a young thesis-native pure-play. See trends.md T27. This doesn't disqualify capital-structure angles outright (a one-time test-and-log pass is still worth it — Wuxi Quanzhibo and Bowintec both originated from investor-adjacent disclosures), but don't allocate a standing rotation slot to one without a specific reason to expect it differs from this pattern.
